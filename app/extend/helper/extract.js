...
    const { field, isRequired = false, defaultValue, errMsg } = rule;
    
    // 参数未定义
...
  includesChecker(val, rule) {
    ...
            const msg = `参数：${field} 必须为以下元素之一 ${JSON.stringify(includes)}, 当前值为${val}`;
            throw new Error(msg);
          }
        }
      },
    ...
        let temp = rawValue;
        const { validate, convert } = this.typeCheckers[type];
        
        // 如果需要转换，先转换再校验
        if (convert) {
    ...
        } catch (e) {
          const { message } = e;
          const printVal = is.object(rawValue) ? JSON.stringify(rawValue) : rawValue;
          const msg = errMsg || `${message}, 当前值为${printVal}，类型为 ${typeof rawValue}`;
...
  extract(data, rules = []) {
    ...
          } = rule;
          const rawValue = data[field]; // 原始值
          
          // 有自定义校验方法，统一走该方法，其他校验不参与
          if (validate) {
